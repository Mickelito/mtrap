...
def get_changes_in_interval(oldest: dt.datetime, newest: dt.datetime, api: str):
    """
    Count occurences of "NEW", "MERGED", and "ABANDONED"
    among changes submitted between 'oldest' and 'newest', from the URL 'api'
    """
    lim_num_changes = 0
    if "android" in api:
...
def plot_changes(time_points, data_points ,source:str ,no_days:int, now_time:dt.datetime, save_path:str):
    total_plot_name = now_time.strftime("%y%m%d_%H%M%S")+"_total_"+source+"_"+str(no_days)+"d.pdf"
    new_plot_name = now_time.strftime("%y%m%d_%H%M%S")+"_new_"+source+"_" +str(no_days)+"d.pdf"
    merged_plot_name = now_time.strftime("%y%m%d_%H%M%S")+"_merged_"+source+"_"+str(no_days)+"d.pdf"
    abandoned_plot_name = now_time.strftime("%y%m%d_%H%M%S")+"_abandoned_"+source+"_"+str(no_days)+"d.pdf"
...
def predict_word(input_word:str, available_words: list):
    """
    Sees if 'input' is substring of and begins with same letter as word in 'available_words'
    Does not work if multiple words start with same substrings
    Made specifically for 'mtrap', which only has three available words
    """
    if not input:
        return -2
    for word in available_words:
        if input_word.lower()[0] == word[0] and input_word.lower() in word:
            return word
    return -1

def check_dir(dir_path:str):
    """
    Checks if a directory exists in 'dir_path'
    Returns 1 if directory does not exist,
        -1 if file with that name exists
    """
...
def check_number_of_days(input_num: str):
    """
    Checks that the entered number of days is valid
    """
    if not input_num:
        return -2
    for i in input_num:
...
def write_log(now:dt.time, source:str, no_days:int, log_dir:str, log_data):
    log_name = now.strftime("%y%m%d_%H%M%S")+"_log_"+source+"_"+str(no_days)+"d.json"
...
def main():
    ...
    while True:
        ...
        plot_changes(data_pts, time_pts, api, time, now, plots)
        write_log(now, api, time, logs, json_log)
